...
"""
pix2gestalt Latent Diffusion Start
"""
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        # xc.shape: torch.Size([192, 3, 256, 256])
        cond_visible_masks = super().get_input(batch, 'visible_mask_cond').to(self.device)
        # cond_visible_masks.shape: torch.Size([192, 3, 256, 256])

        if bs is not None:
            xc = xc[:bs]
            cond_visible_masks = cond_visible_masks[:bs]
        cond = {}
        ...
        with torch.enable_grad():
            ...
            # Conditioning Channel 1: CLIP(input_im)
        ...
        # Conditioning Channel 2: VAE(input_im) + VAE(visible_mask) + Z_t
        xc_encoding = self.encode_first_stage(xc.to(self.device)).mode().detach() # torch.Size([192, 4, 32, 32])
        visible_mask_encoding = self.encode_first_stage(cond_visible_masks.to(self.device)).mode().detach() # torch.Size([192, 4, 32, 32])
        c_concat = torch.cat((xc_encoding, visible_mask_encoding), dim = 1) # torch.Size([192, 8, 32, 32])
        cond["c_concat"] = [input_mask * c_concat] # torch.Size([192, 8, 32, 32])
    ...
    def get_unconditional_conditioning(self, batch_size, null_label=None, image_size=512):
        ...
        cond = {}
        cond["c_crossattn"] = [c]
        cond["c_concat"] = [torch.zeros([batch_size, 8, image_size // 8, image_size // 8]).to(self.device)]
        return cond
    ...
    def to_rgb(self, x):
        ...
        return x
"""
pix2gestalt Latent Diffusion End
"""
